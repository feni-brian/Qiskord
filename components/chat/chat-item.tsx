...
const roleIconMap = { GUEST: null, MODERATOR: <ShieldCheck className="h-4 w-4 ml-2 text-indigo-500" />, ADMIN: <ShieldAlert className="h-4 w-4 ml-2 text-rose-500" /> };
		...
		<div className="relative group flex items-center hover:bg-black/5 p-4 transition w-full">
			<div className="group flex gap-x-2 items-start w-full">
				<div onClick={onMemberClick} className="cursor-pointer hover:drop-shadow-md transition">
					<UserAvatar srcProp={member.profile.imageUrl} />
				</div>
				<div className="flex flex-col w-full">
					...
					{!fileUrl && isEditing && (
						<Form {...form}>
							...
							<span className="text-[10px] mt-1 text-zinc-400">Press `Escape` to cancel, or, `Enter` to save!</span>
						</Form>
					)}
				</div>
			</div>
